...
mod test {
    ...
    fn get_antennas(lines: &Vec<String>) -> HashMap<char, Vec<(i32, i32)>> {
        ...
        for (row, line) in lines.iter().enumerate() {
            for (col, c) in line.chars().enumerate() {
                if c != '.' {
                    antennas.entry(c).or_insert(vec![]).push((row as i32, col as i32));
                }
            }
        }
        antennas
    ...
    fn get_antinodes(bounds: &(i32,i32), points: &Vec<(i32, i32)>, include_resonance: bool) -> Vec<(i32, i32)> {
        ...
        for i in 0..points.len() {
            for j in i+1..points.len() {
                ...
                loop {
                    ...
                    if next_left.0 < 0 || next_left.1 < 0 || next_left.0 >= bounds.0 || next_left.1 >= bounds.1  {
                        break;
                    }
                ...
                loop {
                    ...
                    if next_right.0 < 0 || next_right.1 < 0 || next_right.0 >= bounds.0 || next_right.1 >= bounds.1  {
                        break;
                    }
    ...
    fn day8_part1(){
        ...
        for (_, points) in antennas.iter() {
            let antinodes:Vec<(i32, i32)> = get_antinodes(&bounds, &points, false).iter().cloned().filter(|(x, y)| x >= &0 && y >= &0 && x < &bounds.0 && y < &bounds.1).collect();
    ...
    fn day8_part2(){
        ...
        for (_, points) in antennas.iter() {
            let antinodes:Vec<(i32, i32)> = get_antinodes(&bounds, &points, true).iter().cloned().filter(|(x, y)| x >= &0 && y >= &0 && x < &bounds.0 && y < &bounds.1).collect();
            antinodeset.extend(antinodes.iter().cloned());
        }

        //Antinodes also occur on every antenna
        for (_, points) in antennas.iter() {
